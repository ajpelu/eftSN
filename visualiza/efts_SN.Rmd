...
m <- mb %>% 
  ...
  addRasterImage(eftFactor, group = "EFT",
                 colors = eft_palette$eft_color, opacity = 0.6) %>% 
...
mapita <- tm_basemap(c("Esri.WorldImagery", "Esri.WorldGrayCanvas", "Esri.WorldTopoMap")) +
  # eft 
  ...
  tm_raster(alpha=.7, legend.is.portrait = FALSE, colorNA = NULL, 
            palette = rainbow(10, rev = TRUE), style = "cont", 
...
#               group = "EFT") +
 +
  ...
  tm_raster(alpha=.7, palette = evi_mmax_palette$color, legend.is.portrait = FALSE,
            group = "EVI mMax") +
