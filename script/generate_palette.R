...
eftLeg <- efts %>% 
  unite("eft_code", c("productivity", "seasonality", "phenology"), sep="", remove = FALSE) %>% 
  mutate(idd = paste0(eft_id, ") ")) %>% 
  unite("eft_nameLegend", c("idd", "eft_code"), sep = " ", remove = FALSE) %>% 
  mutate(eft_color = rgb(r = r,g = g,b = b, maxColorValue = 255)) %>% 
  dplyr::select(eft_id, eft_code, eft_color, eft_nameLegend, productivity:column)
